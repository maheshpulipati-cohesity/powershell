...
"View Name,Logical(GB),Storage Consumed(GB),Data Written (GB),Resiliency (GB),Logical Quota (GB),QOS Type,Protocol Supported,View ID" | Out-File -FilePath $outfileName
...
$LogicalQuota = $view.views[${index}].logicalQuota.hardLimitBytes
...
$LogicalQuotaGB = [math]::round($LogicalQuota/1073741824, 2)

"$name,$LogicalGB,$StorageConsumedGB,$DataWrittenGB,$ResiliencyGB,$LogicalQuotaGB,$Qos,$Protocol,$ViewId" | Out-File -FilePath $outfileName -Append
}
